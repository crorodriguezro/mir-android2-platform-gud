...
Both PPM files must be in P6 format (binary RGB, 3 bytes/pixel) and must
have identical dimensions.  The frames should originate from the same
reference source (e.g. generated by `mirgud --quality --dump-frame`) so that
differences reflect pixel-format quantization, not scene changes.
...
Reports:
    ...
    - Gradient continuity diagnostic (consecutive identical rows/columns)
"""
...
def compare(path_a, path_b):
    ...
    # PSNR: peak signal-to-noise ratio (255 is max pixel value)
    if rmse > 0:
    ...
    if psnr == float('inf'):
        print(f"  PSNR=inf (identical)")
    else:
        print(f"  PSNR={psnr:.2f} dB")

    # Gradient continuity diagnostic: count fully-identical rows/columns.
    # This is a diagnostic, NOT a measure of banding severity.
    band_rows = 0
    band_cols = 0
    row_pixel = 3 * w_a
    for y in range(h_a):
    ...
    print(f"  Diagnostic: {band_rows} fully-identical rows, "
          f"{band_cols} fully-identical columns")
