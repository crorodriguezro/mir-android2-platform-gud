...
TEST_F(Display, preserves_external_only_power_policy_through_configuration)
{
    using namespace testing;
    stub_db_factory->with_next_config([&](mtd::MockHwcConfiguration& mock_config)
    {
    ...
    configuration->for_each_output([&](mg::UserDisplayConfigurationOutput& output)
    {
        ...
        else if (output.id == external_output_id)
        {
            output.used = true;
        }
    });
    ...
    auto effective_configuration = display.configuration();
    effective_configuration->for_each_output([&](mg::UserDisplayConfigurationOutput const& output)
    {
        if (output.id == primary_output_id)
        {
        ...
        {
            EXPECT_THAT(output.power_mode, Eq(mir_power_mode_on));
        }
    });
    ...
    {
        group.for_each_display_buffer([&](mg::DisplayBuffer&) { ++compositor_targets; });
    });
...
TEST_F(Display, leaves_primary_power_requested_by_used_primary_configuration_unchanged)
{
    ...
    effective_configuration->for_each_output([&](mg::UserDisplayConfigurationOutput const& output)
    {
        if (output.id == primary_output_id)
        {
            EXPECT_THAT(output.power_mode, Eq(mir_power_mode_suspend));
        }
    });
}
