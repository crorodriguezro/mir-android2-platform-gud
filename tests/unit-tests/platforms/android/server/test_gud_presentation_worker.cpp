...
TEST(GudHwcBoundary, excludes_only_synthetic_external_from_android_hwc)
{
    ...
    EXPECT_TRUE(mga::affects_hwc_pacing(true, mga::DisplayName::primary));
    EXPECT_FALSE(mga::affects_hwc_pacing(true, mga::DisplayName::external));
    EXPECT_TRUE(mga::affects_hwc_pacing(true, mga::DisplayName::virt));
    EXPECT_TRUE(mga::affects_hwc_pacing(false, mga::DisplayName::external));
}
...
TEST(GudSyntheticOutputControl, leaves_gud_lifecycle_to_the_dedicated_service)
{
    EXPECT_FALSE(mga::should_expose_synthetic_gud_output());
    EXPECT_FALSE(mga::should_bypass_synthetic_hwc_bookkeeping());
    EXPECT_TRUE(mga::should_offer_synthetic_output_to_compositor());
    EXPECT_TRUE(mga::should_mark_primary_output_used());
    EXPECT_FALSE(mga::should_mark_synthetic_output_used());
}
